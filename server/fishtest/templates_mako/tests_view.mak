...
<div id="enclosure"${' style="visibility:hidden;"' if show_task >= 0 else "" |n}>
  ...
  <h2>
    ...
    <a href="${helpers.diff_url_for_run(run, allow_github_api_calls)}" target="_blank" rel="noopener">diff</a>
  </h2>
    ...
    <div class="col-12 col-lg-9">
      <div id="diff-section">
        <h4>
          ...
          <a
            href="${helpers.diff_url_for_run(run, allow_github_api_calls)}"
  ...
  async function handleDiff() {
    ...
    if (
      ...
      copyDiffBtn.addEventListener("click", () => {
        ...
        textarea.textContent = "curl -s ${helpers.diff_url_for_run(run, allow_github_api_calls)}.diff | git apply";
    ...
    const diffApiUrl = "${helpers.diff_url_for_run(run, allow_github_api_calls)}".replace(
      "//github.com/",
    ...
    const testRepo = "${helpers.tests_repo(run)}";
