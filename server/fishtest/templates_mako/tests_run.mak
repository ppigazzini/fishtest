...
<%!
  ...
  fb = lambda e0, e1: helpers.format_bounds(elo_model, e0, e1)
  test_book = "UHO_Lichess_4852_v1.epd"
  pt_book = "UHO_Lichess_4852_v1.epd"
%>
<%
  setup = helpers.tests_run_setup(args, master_info, pt_info, test_book)
  base_branch = setup["base_branch"]
  latest_bench = setup["latest_bench"]
  pt_version = setup["pt_version"]
  pt_branch = setup["pt_branch"]
  pt_signature = setup["pt_signature"]
  tc = setup["tc"]
  new_tc = setup["new_tc"]
  default_book = setup["default_book"]
  is_odds = setup["is_odds"]
  arch_filter = setup["arch_filter"]
