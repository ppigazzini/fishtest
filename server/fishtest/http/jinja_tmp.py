"""Jinja2 template rendering helpers for the FastAPI UI (tmp set)."""
...
TEMPLATES_DIR_ENV: Final[str] = "FISHTEST_JINJA_TMP_TEMPLATES_DIR"
...
def templates_dir() -> Path:
    """Return the Jinja2 templates directory path for the tmp set."""
    raw = environ.get(TEMPLATES_DIR_ENV, "").strip()
    if raw:
        return Path(raw)
    return _repo_root() / "server" / "fishtest" / "templates_jinja2_tmp"
...
def default_environment() -> Environment:
    """Return a Jinja2 environment bound to the tmp templates directory."""
    env = Environment(
...
def default_templates() -> Jinja2Templates:
    """Return a Starlette Jinja2Templates instance for tmp templates."""
