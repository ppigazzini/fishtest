...
class TestHttpUsers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = self.fastapi_util.make_test_client(
            rundb=self.rundb,
            include_api=False,
            include_views=True,
        )
    ...
    def tearDownClass(cls):
        cls.rundb.userdb.users.delete_many(
            {"username": {"$in": [cls.username, cls.signup_username]}},
        )
        cls.rundb.userdb.user_cache.delete_many(
            {"username": {"$in": [cls.username, cls.signup_username]}},
        )
