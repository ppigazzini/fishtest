...
def run_single_bench(engine, hash, threads, depth):
    bench_time, bench_nodes = None, None
    try:
        p = subprocess.Popen(
            [engine, "bench", str(hash), str(threads), str(depth), "default", "depth"],
...
def run_parallel_benches(engine, concurrency, hash, threads, depth):
...
def get_bench_nps(engine, games_concurrency, hash, threads):
    # Preload the CPU then get the NPS with the required hash, threads and depth
    _depth, depth = 6, 13
    _ = run_parallel_benches(engine, games_concurrency, hash, threads, _depth)
    results = run_parallel_benches(engine, games_concurrency, hash, threads, depth)
...
def verify_signature(engine, signature):
    hash, threads, depth = 16, 1, 13
    _, bench_nodes = run_single_bench(engine, hash, threads, depth)
...
def run_games(
    ...
    try:
        ...
        base_nps = get_bench_nps(base_engine, games_concurrency, base_hash, threads)
    ...
    if not (
        ...
        try:
            ...
            _ = get_bench_nps(new_engine, games_concurrency, new_hash, threads)
        except RunException as e:
            run_errors.append(str(e))
        except WorkerException as e:
            raise e
