...
- [x] FastAPI HTTP contract tests exist for worker endpoints in [server/tests/test_api_fastapi.py](../../server/tests/test_api_fastapi.py).
...
- [x] Regression tests cover JSON-vs-HTML and worker validation shaping in [server/tests/test_glue_errors_fastapi.py](../../server/tests/test_glue_errors_fastapi.py).

### Phase 3 — UI session + CSRF semantics gates

- [x] UI flows are exercised by FastAPI tests (auth basics) in [server/tests/test_users_fastapi.py](../../server/tests/test_users_fastapi.py).
- [x] UI session/CSRF invariants are regression-tested in [server/tests/test_ui_session_semantics_fastapi.py](../../server/tests/test_ui_session_semantics_fastapi.py).
