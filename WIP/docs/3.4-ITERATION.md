...
- **Gates** (targeted):
   - From `server/`: `uv run python -m unittest discover -s tests -q`
   - If touching error shaping: ensure [server/tests/test_glue_errors_fastapi.py](../../server/tests/test_glue_errors_fastapi.py) stays green.
...
12) (Optional) Extend the primary-only guard beyond workers: identify UI mutation (POST) endpoints that should require a primary instance.
      ...
      - Explicit non-primary exceptions (must be safe to serve from a non-primary):
         - `/tests` → `backend_8001`
      ...
      - UI mutation paths that are **NOT** primary-routed by the current Nginx map and therefore are the highest-risk candidates:
         - `upload` (`/upload`): calls `request.rundb.upload_nn(...)` but currently falls under `default → backend_8001`.
...
- **Gates**:
   - From `server/`: `uv run python -m unittest discover -s tests -q`
   - Ensure worker contract tests remain green (see [server/tests/test_api_fastapi.py](../../server/tests/test_api_fastapi.py)).
