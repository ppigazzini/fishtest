...
## Current refactor status (2026-02-05)

- Legacy Mako remains the parity anchor; only minimal, ASCII-safe fixes are applied to upstream (entities for symbols, tag fixes) before the refactor.
- New Mako and Jinja2 outputs match legacy Mako (normalized parity OK).
- Renderer selection is set via a Python variable in the renderer module (no environment flags).
