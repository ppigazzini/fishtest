# Iteration plan (Milestone 6 — Contract tests & de‑Pyramidized tests)

Date: 2026-01-29

This iteration defines Milestone 6: migrate tests off Pyramid stubs, consolidate contract tests for worker and UI protocols, and ensure CI validates the active FastAPI HTTP surface.
...
Metrics
- Zero imports from `pyramid.*` in `server/tests`.
...
- Documented the completed test migration effort in the parity report and linked tools; no `pyramid.*` imports remain under `server/tests`.
