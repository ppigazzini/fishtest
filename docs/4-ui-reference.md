...
| `/tests/view/{id}` | GET | `tests_view` | `tests_view.html.j2` | Full detail page; renders server-owned Open Graph metadata with a query-free canonical URL; unfinished runs poll the merged detail fragment endpoint and the dedicated tasks endpoint |
