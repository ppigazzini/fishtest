...
# At 20k workers the primary needs ~15k fds; 32768 provides 2× headroom.
LimitNOFILE=32768
...
server {
    ...
    # block bad actors at the server level (early acces phase)
...
http {
    ...
    gzip_proxied any; # required to compress responses from python backends
...
`worker_connections 32768` provides 1.6× headroom per worker.
...
# 20k client + 20k upstream + mongodb + misc ≈ 45k peak; 120k provides ~2.5× headroom
fs.file-max = 120000
...
provides 2× headroom and absorbs a full thundering-herd reconnection burst.
