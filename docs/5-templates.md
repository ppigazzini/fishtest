...
- `run_tables_ctx` is intentionally lightweight. Unfinished-run rows omit
   ...
   `args.spsa.param_history`; task detail stays on the detail and tasks routes.
...
| `form_action` | string |
| `spsa_form_values` | dict (`algorithm`, `raw_params`, `sf_lr`, `sf_beta1`, `sf_beta2`, `sf_eps`) |
...
- Reruns reuse the stored raw parameter rows and preserve SF-Adam `sf_lr`, `sf_beta1`, `sf_beta2`, and `sf_eps` when they exist.
...
- `static/js/spsa.js` treats that payload as renderer-ready and does not
   rebuild optimizer history or live points in the browser
